...
#    2          Button input, screen/light handing and cloud
#               function added
#------------------------------------------------------------------
...
class TemperatureMachine(StateMachine):
    ...
    # Track whether the system is actively heating or cooling
    equip_state = "off"
    ...
    def processTempIncButton(self):
        oldSetPoint = self.setPoint
        self.setPoint +=1
        self.updateLights()
        
        if(DEBUG):
            print("Increasing Set Point")
            print(f"{oldSetPoint} to {self.setPoint}")
    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            blueLight.off()
            if temp < self.setPoint and self.equip_state == "off":
                redLight.pulse()
                self.equip_state = "heating"
            elif temp < self.setPoint and self.equip_state == "heating":
                # LED has already been set to pulse
                pass
            else:
                # Temp set point has been reached
                redLight.on()
                self.equip_state = "off"
        ...
        if self.current_state.id == "cool":
            ...
            if temp > self.setPoint and self.equip_state == "off":
                blueLight.pulse()
                self.equip_state = "cooling"
            elif temp > self.setPoint and self.equip_state == "cooling":
                # LED has already been set to pulse
                pass
            else:
                # Temp set point has been reached
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = str(current_time)
            
            ## Setup Display Line 2
            if(altCounter < 6):
                temp = floor(self.getFahrenheit())
                lcd_line_2 = f"Current Temp:{temp}"
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"{self.current_state.id.capitalize()} Set: {self.setPoint}"
